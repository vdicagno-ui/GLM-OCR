...
def make_template(path: Path):
    ...
    doc.add_paragraph(
        "Il sottoscritto [da compilare nome e cognome], in qualità di difensore,"
    )
    ...
    tbl.rows[0].cells[1].text = "[da compilare nome e cognome]"
...
def make_guide(path: Path, nome, rg, data, foro):
    ...
    doc.add_paragraph(f"Nome e cognome: {nome}")
